create table linguistic.evidence_lifecycle_events (
  ...
  constraint ele_sequence_check check (
    event_sequence > 0
  ),

  constraint ele_action_check check (
    action_code in ('retracted', 'reinstated')
  ),

  constraint ele_reason_check check (
    char_length(reason) between 1 and 2000
    and reason !~ '^[[:space:]]'
    and reason !~ '[[:space:]]$'
  ),

  constraint ele_documentary_provenance_fkey foreign key (
    target_documentary_provenance_link_id
  ) references linguistic.documentary_provenance_links (id)
    on update restrict
    on delete restrict,

  constraint ele_form_attestation_fkey foreign key (
    target_form_attestation_id
  ) references linguistic.form_attestations (id)
    on update restrict
    on delete restrict,

  constraint ele_review_actor_fkey foreign key (review_actor_id)
    references linguistic.review_actors (id)
    on update restrict
    on delete restrict
);

create unique index ele_dpl_sequence_uidx
  on linguistic.evidence_lifecycle_events (
    target_documentary_provenance_link_id,
    event_sequence
  )
  where target_documentary_provenance_link_id is not null;

create unique index ele_attestation_sequence_uidx
  on linguistic.evidence_lifecycle_events (
    target_form_attestation_id,
    event_sequence
  )
...
create index ele_review_actor_created_idx
  on linguistic.evidence_lifecycle_events (
    review_actor_id,
    created_at,
    id
...
create function linguistic.guard_evidence_lifecycle_event_insert()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
create function linguistic.guard_evidence_lifecycle_event_mutation()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using
    errcode = '55000',
    message = 'evidence lifecycle events are append-only';
end;
$$;

create function linguistic.guard_documentary_provenance_link_delete()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using
    errcode = '55000',
    message = 'documentary provenance links cannot be deleted; append a lifecycle retraction instead';
end;
$$;

create function linguistic.guard_form_attestation_delete()
returns trigger
language plpgsql
security invoker
set search_path = ''
...
begin
  raise exception using
    errcode = '55000',
    message = 'form attestations cannot be deleted; append a lifecycle retraction instead';
end;
$$;

create trigger guard_evidence_lifecycle_events_insert
before insert on linguistic.evidence_lifecycle_events
for each row
execute function linguistic.guard_evidence_lifecycle_event_insert();

create trigger guard_evidence_lifecycle_events_append_only
before update or delete on linguistic.evidence_lifecycle_events
for each row
execute function linguistic.guard_evidence_lifecycle_event_mutation();

create trigger guard_dpl_delete
before delete on linguistic.documentary_provenance_links
for each row
execute function linguistic.guard_documentary_provenance_link_delete();

create trigger guard_form_attestations_delete
before delete on linguistic.form_attestations
for each row
execute function linguistic.guard_form_attestation_delete();

alter table linguistic.evidence_lifecycle_events enable row level security;

revoke all on table linguistic.evidence_lifecycle_events
  from public, anon, authenticated, service_role;

revoke execute on function linguistic.guard_evidence_lifecycle_event_insert()
  from public, anon, authenticated, service_role;

revoke execute on function linguistic.guard_evidence_lifecycle_event_mutation()
  from public, anon, authenticated, service_role;

revoke execute on function linguistic.guard_documentary_provenance_link_delete()
  from public, anon, authenticated, service_role;

revoke execute on function linguistic.guard_form_attestation_delete()
  from public, anon, authenticated, service_role;
